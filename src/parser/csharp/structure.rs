...
use tree_sitter::{Node, Query, QueryCursor, QueryError, QueryMatch, StreamingIterator, Tree};
...
fn evaluate_ns_decl<'t, 'b>(
    node: Node<'t>,
    qmatch: &QueryMatch<'_, 't>,
    buffer: &'b [u8],
    result: &mut StructureInfo<'b, 't>,
) -> Result<(), Error> {
...
fn evaluate_ns_usage<'t, 'b>(
    ...
    let decl_node = match id_node.parent() {
        Some(p) => p,
        None => {
            return Err(Error::BadStaticUsing(
                id_node
                    .utf8_text(buffer)
                    .map(|s| s.to_string())
                    .map_err(|e| Error::Utf8(e))?,
    ...
    if let Some(alias) = alias {
        ...
        result.ns_usages.entry(scope_node).or_insert(HashSet::new()).insert(id);
    }
    ...
    let (name, node) = match name_parts {
        (Some(id), Some(generics)) => (
            QualifiedNameRef::from(
                str::from_utf8(&buffer[id.start_byte()..generics.end_byte()]).map_err(|e| Error::Utf8(e))?,
            ),
            id,
        ),
        (Some(id), None) => (
...
fn evaluate_type_usage<'t, 'b>(
    node: Node<'t>,
    _qmatch: &QueryMatch<'_, 't>,
    buffer: &'b [u8],
    result: &mut StructureInfo<'b, 't>,
) -> Result<(), Error> {
    // skip "using x = typename", included in ns_usage
    if let Some(user) = node.parent()
        && user.kind_id() == *k::USING
    {
        return Ok(());
    }
...
fn evaluate_var_decl<'t, 'b>(
    ...
    result.var_decl.entry(node).or_insert(HashSet::new()).insert(id);
    Ok(())
}
...
mod test {
    ...
    fn assert_map<'t, T, U, I>(actual: HashMap<Node<'t>, I>, expected: HashMap<T, I>)
    where
        T: From<Node<'t>> + PartialEq<Node<'t>> + Debug + Clone + Eq + Hash + Display,
    ...
    fn evaluate_structure_ns() {
        let result = super::evaluate_structure(&NS_TEST_TREE, NS_TEST_CODE).expect("Evaluation failed");

        assert_eq!(result.fsns_decl, Some(QualifiedNameRef::from("L0")));

        assert_map(
            result.ns_decl_names,
            HashMap::from([
                (ROOT.clone(), HashSet::from([QualifiedNameRef::from("L1")])),
                (
                    NodeLike::new("declaration_list", 6, 0),
                    HashSet::from([QualifiedNameRef::from("L2"), QualifiedNameRef::from("L3")]),
                ),
            ]),
        );

        assert_map(
            result.ns_decl_nodes,
            HashMap::from([
                (
                    NodeLike::new("namespace_declaration", 5, 0),
                    QualifiedNameRef::from("L1"),
                ),
                (
                    NodeLike::new("namespace_declaration", 11, 4),
                    QualifiedNameRef::from("L2"),
                ),
                (
                    NodeLike::new("namespace_declaration", 20, 4),
        ...
        assert_map(
            result.ns_usages,
            HashMap::from([
                (ROOT.clone(), HashSet::from([QualifiedNameRef::from("Ns0")])),
                (
                    NodeLike::new("declaration_list", 6, 0),
                    HashSet::from([QualifiedNameRef::from("Ns1")]),
                ),
                (
                    NodeLike::new("declaration_list", 12, 4),
                    HashSet::from([QualifiedNameRef::from("Ns2")]),
                ),
            ]),
        );
        ...
        assert_map(
            result.type_decl_names,
            HashMap::from([
                (
                    NodeLike::new("declaration_list", 12, 4),
                    HashSet::from([QualifiedNameRef::from("Class2")]),
                ),
                (
                    NodeLike::new("declaration_list", 21, 4),
                    HashSet::from([QualifiedNameRef::from("Class3")]),
                ),
            ]),
        );

        assert_map(
            result.type_decl_nodes,
            HashMap::from([
                (
                    NodeLike::new("class_declaration", 17, 8),
                    QualifiedNameRef::from("Class2"),
                ),
                (
                    NodeLike::new("class_declaration", 22, 8),
        ...
        assert_map(
            result.type_usages,
            HashMap::from([(
                NodeLike::new("qualified_name", 17, 23),
                QualifiedNameRef::from("L3.Class3"),
            )]),
        );
    ...
    fn evaluate_structure_type() {
        let result = super::evaluate_structure(&TYPE_TEST_TREE, TYPE_TEST_CODE).expect("Evaluation failed");
        ...
        assert_map(
            ...
            HashMap::from([(
                NS1.clone(),
                HashMap::from([(QualifiedNameRef::from("ns3a"), QualifiedNameRef::from("Ns3"))]),
            )]),
        );
        ...
        assert_map(
            ...
            HashMap::from([
                (
                    ROOT.clone(),
                    HashSet::from([QualifiedNameRef::from("Ns1"), QualifiedNameRef::from("Ns3")]),
                ),
                (NS1.clone(), HashSet::from([QualifiedNameRef::from("Ns2")])),
            ]),
        );
        ...
        assert_map(
            ...
            HashMap::from([
                (
                    NodeLike::new("namespace_declaration", 7, 0),
                ...
                (
                    NodeLike::new("namespace_declaration", 35, 0),
        ...
        assert_map(
            result.type_decl_names,
            HashMap::from([
                (ROOT.clone(), HashSet::from([QualifiedNameRef::from("Enum0")])),
                (
                    NS1.clone(),
                    HashSet::from([QualifiedNameRef::from("Struct1<T>"), QualifiedNameRef::from("Class1")]),
                ),
                (
                    NodeLike::new("declaration_list", 12, 4),
                    HashSet::from([QualifiedNameRef::from("Record2")]),
                ),
                (
                    NodeLike::new("struct_declaration", 16, 4),
                    HashSet::from([QualifiedNameRef::from("T")]),
                ),
                (
                    NodeLike::new("declaration_list", 22, 4),
                    HashSet::from([QualifiedNameRef::from("ChildClass")]),
                ),
                (
                    NodeLike::new("declaration_list", 36, 0),
                    HashSet::from([QualifiedNameRef::from("INterface3")]),
                ),
            ]),
        );
        ...
        assert_map(
            ...
            HashMap::from([
                ...
                (
                    NodeLike::new("record_declaration", 13, 8),
                ...
                (
                    NodeLike::new("class_declaration", 21, 4),
                    QualifiedNameRef::from("Class1"),
                ),
                (
                    NodeLike::new("class_declaration", 23, 8),
                    QualifiedNameRef::from("ChildClass"),
                ),
                (
                    NodeLike::new("interface_declaration", 37, 4),
        ...
        assert_map(
            ...
            HashMap::from([
                ...
                (
                    NodeLike::new("identifier", 25, 15),
                    QualifiedNameRef::from("ChildClass"),
                ),
                (
                    NodeLike::new("generic_name", 27, 15),
                    QualifiedNameRef::from("Struct1<ns3a::INterface3>"),
                ),
                (
                    NodeLike::new("alias_qualified_name", 27, 23),
                    QualifiedNameRef::from("ns3a::INterface3"),
                ),
                (NodeLike::new("identifier", 29, 15), QualifiedNameRef::from("Enum0")),
                (
                    NodeLike::new("qualified_name", 31, 15),
                    QualifiedNameRef::from("Ns2.Record2"),
                ),
            ]),
        );
        ...
        assert_map(
            ...
            HashMap::from([
                (
                    NodeLike::new("declaration_list", 17, 4),
                    HashSet::from([QualifiedNameRef::from("Value")]),
                ),
                (
                    NodeLike::new("declaration_list", 22, 4),
                    HashSet::from([
    ...
    fn evaluate_structure_var() {
        let result = super::evaluate_structure(&VAR_TEST_TREE, VAR_TEST_CODE).expect("Failed to evaluate structure");
        ...
        assert_map(
            ...
            HashMap::from([
                (
                    ROOT.clone(),
                    HashMap::from([(QualifiedNameRef::from("X"), QualifiedNameRef::from("Ns1.Class2"))]),
                ),
                (
                    NodeLike::new("declaration_list", 3, 0),
                    HashMap::from([(QualifiedNameRef::from("Y"), QualifiedNameRef::from("Ns1.Class3"))]),
                ),
            ]),
        );
        ...
        assert_map(
            ...
            HashMap::from([(
                ROOT.clone(),
                HashSet::from([QualifiedNameRef::from("Ns0"), QualifiedNameRef::from("Ns1")]),
            )]),
        );
        assert_map(
            result.ns_decl_nodes,
            HashMap::from([
                (
                    NodeLike::new("namespace_declaration", 2, 0),
                    QualifiedNameRef::from("Ns0"),
                ),
                (
                    NodeLike::new("namespace_declaration", 33, 0),
                    QualifiedNameRef::from("Ns1"),
                ),
            ]),
        );
        ...
        assert_map(
            ...
            HashMap::from([(
                NodeLike::new("declaration_list", 3, 0),
                HashSet::from([QualifiedNameRef::from("System.Text")]),
            )]),
        );
        ...
        assert_map(
            ...
            HashMap::from([
                (
                    NodeLike::new("declaration_list", 3, 0),
                    HashSet::from([QualifiedNameRef::from("Delegate1"), QualifiedNameRef::from("Class1")]),
                ),
                (
                    NodeLike::new("declaration_list", 34, 0),
                    HashSet::from([QualifiedNameRef::from("Class2"), QualifiedNameRef::from("Class3")]),
                ),
            ]),
        );
        ...
        assert_map(
            ...
            HashMap::from([
                (
                    NodeLike::new("delegate_declaration", 7, 4),
                    QualifiedNameRef::from("Delegate1"),
                ),
                (
                    NodeLike::new("class_declaration", 9, 4),
                    QualifiedNameRef::from("Class1"),
                ),
                (
                    NodeLike::new("class_declaration", 35, 4),
                    QualifiedNameRef::from("Class2"),
                ),
                (
                    NodeLike::new("class_declaration", 36, 4),
                    QualifiedNameRef::from("Class3"),
                ),
            ]),
        );
        ...
        assert_map(
            ...
            HashMap::from([
                ...
                (
                    NodeLike::new("identifier", 21, 19),
                    QualifiedNameRef::from("StringBuilder"),
                ),
            ]),
        );
        ...
        assert_map(
            ...
            HashMap::from([
                (
                    NodeLike::new("declaration_list", 10, 4),
                    HashSet::from([
                ...
                (
                    NodeLike::new("method_declaration", 17, 8),
                    HashSet::from([QualifiedNameRef::from("count")]),
                ),
                (
                    NodeLike::new("block", 18, 8),
                    HashSet::from([QualifiedNameRef::from("x"), QualifiedNameRef::from("test")]),
                ),
                (
                    NodeLike::new("using_statement", 20, 12),
                    HashSet::from([QualifiedNameRef::from("test")]),
                ),
                (
                    NodeLike::new("using_statement", 21, 12),
                    HashSet::from([QualifiedNameRef::from("sb")]),
                ),
                (
                    NodeLike::new("for_statement", 23, 16),
                    HashSet::from([QualifiedNameRef::from("i")]),
                ),
            ]),
        );
        ...
        assert_map(
            ...
            HashMap::from([
                ...
                (
                    NodeLike::new("identifier", 21, 38),
                    QualifiedNameRef::from("Ns.Main.StringBuilderCache"),
                ),
